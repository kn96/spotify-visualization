...
function SpotifyWebApi(credentials) {
  ...
  this.getPlaylistTracks = function(userId, playlistId, options, callback) {
    ...
    var getAllTracks = function (request) {
      return new Promise(function (resolve, reject) {
        var tracks = []

        var recursivelyGetTracks = function(nextPage) {
          request.path = nextPage.match(/\/v1[\s\S]+/)[0]
          _performRequest(HttpManager.get, request).then(function (data) {
            tracks = tracks.concat(data.body.items)
            if (data.body.next === null) {
              resolve(tracks)
            } else {
              recursivelyGetTracks(data.body.next)
            }
          })
        }

        recursivelyGetTracks('/v1/users/' + encodeURI(userId) + '/playlists/' + playlistId + '/tracks')
      })
    }
